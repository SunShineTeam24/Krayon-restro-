    ...
    const selectPage = (page) => {
      ...
      const getCustomerPointdata = () => {
        ...
        const fetchDataa = async () => {
          try {
            const response = await axios.get(`${API_BASE_URL}/customerpoints`);
        ...
        const downloadExcel = async () => {
          ...
          worksheet.columns = [
            ...
            { header: "Spend Amount", key: "spend amount" },
            { header: "Total Point", key: "total point" },
...
<section className=" tabledata pr-4 pl-4 ">
  ...
  {row.total_bill_amount != null
    ? Number(row.total_bill_amount).toFixed(2)
    : "No Amount found"}
</td>

<td className="py-2 px-4 border border-[#4CBBA1]">
  {row.earning_points != null
    ? Number(row.earning_points).toFixed(2)
    : "No Point"}
</td>
