...
export const ComponentToPrintInvoice = React.forwardRef((props, ref) => {
  ...
  console.log(invoiceData,"invoice data aaya ")
    ...
    <div ref={ref}>
      ...
      {/* <header className="text-center">
        ...
        <table className="w-full text-left">
          ...
          <tbody>
            {invoiceData.orderMenuData.map((item, index) => {
                    ...
                    item.addons.map((addon, aIndex) => (
                      <tr key={aIndex}>
                        ...
                        <td className="py-2 text-right">
                         ${(Number(addon.price) * Number(addon.quantity)).toFixed(2)}
                        </td>
                      </tr>
                    ))}
            ...
            <tr className="font-semibold">
              ...
              <td className="py-2 text-right border-t">
                ${invoiceData.billData.total_amount}
              </td>
            </tr>
            <tr className=" font-semibold">
              <td className="py-2 ">Vat</td>
              <td className="py-2 text-right">
                ${invoiceData.billData.VAT}
              </td>
            </tr>
            <tr className="font-semibold">
              <td className="py-2 border-t">Service Charge</td>
              <td className="py-2 text-right border-t">
                ${invoiceData.billData.service_charge}
              </td>
            </tr>
            <tr className="font-semibold">
              <td className="py-2 border-t">Total Payment</td>
              <td className="py-2 text-right border-t">
                ${invoiceData.billData.bill_amount}
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <p>Billing To: {invoiceData.customerOrderData.customer_name}</p>
          <p>Bill By:{username} </p>
          <p>
            Table: {invoiceData.customerOrderData.table_no} | Order No.:{" "}
            {invoiceData.customerOrderData.order_id}
          </p>
